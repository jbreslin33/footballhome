...
// --- NEW: Auto-generate + preview + confirm ---
  ...
  (async () => {
    ...
    // numbered from posterNum + the live total from postLog so each post
    // says "Day N of M" (the cadence is one poster per day). The total is
    // sourced from postLog.TOTAL_POSTERS, so adding/removing posters in
