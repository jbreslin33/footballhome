...
PersonPayments::loadMembersForProgram(long long programId) {
    ...
    auto rows = db_->query(
        ...
        "   GROUP BY person_id"
        "),"
        "person_emails_with_parent AS ("
        "  SELECT p.id AS person_id,"
        "         COALESCE(NULLIF(pe.email, ''), parent_pe.email) AS email,"
        "         COALESCE(pe.is_primary, false) AS is_primary,"
        "         COALESCE(pe.created_at, parent_pe.created_at) AS created_at,"
        "         COALESCE(pe.id, parent_pe.id) AS email_id"
        "    FROM persons p"
        "    LEFT JOIN person_emails pe ON pe.person_id = p.id"
        "    LEFT JOIN person_emails parent_pe ON parent_pe.person_id = p.parent_person_id AND pe.person_id IS NULL"
        "),"
        "primary_email AS ("
        // Prefer is_primary=true, then most-recently-created. One row per person.
        "  SELECT DISTINCT ON (person_id) person_id, email"
        "    FROM person_emails_with_parent"
        "   ORDER BY person_id, is_primary DESC NULLS LAST, created_at DESC NULLS LAST, email_id DESC"
        "),"
        "primary_phone AS ("
