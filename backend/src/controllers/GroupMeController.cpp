...
Response GroupMeController::handleSyncForMatch(const Request& request) {
    ...
    try {
        ...
        if (!matchResult.empty()) {
            ...
            // external_id IS NOT NULL: skip placeholder events generated by recurring schedule
        ...
        try {
            ...
            if (!clubResult.empty() && !clubResult[0]["club_id"].is_null()) {
                ...
                for (const auto& chatRow : clubChatsResult) {
                    ...
                    std::string tEventsUrl = "https://api.groupme.com/v3/conversations/" + clubGroupId
                                           + "/events/list?token=" + accessToken_;
