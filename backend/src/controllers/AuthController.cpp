...
Response AuthController::handleCoachTeams(const Request& request) {
    try {
        ...
        // gate a team's visibility on board_sort_order IS NOT NULL AND
        // is_active = true (migration 262 — is_active is the "does this
        // team operate" flag; board_archived_at is presentation-only);
        // they don't care about division/conference/season/league at
        // all. So "Select Team to Coach" must filter on the same two
        // columns, or archived internal teams (e.g. the old grassroots
