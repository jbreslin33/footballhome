-- Christian Hernandez (person 22570, LA user 52674863) registered with LeagueApps
-- under christianher7@icloud.com but signs in to FH with Google as
-- chernandez5000@holyfamily.edu.  His first Google login (2026-08-20) minted a blank
-- orphan person (22587, last name only) holding that email + a users row
-- and zero roster ties, so he could not see his team's events.
-- Real row ties: Mens (team_persons row on the mens board, person_la_memberships x1).
...
-- added to 22570 while the orphan still holds it.  Merge the orphan into
...
--
-- Idempotent: no-ops if 22587 is already gone or no longer owns the email.
DO $$
DECLARE
  kept  CONSTANT int := 22570;
  drop_ CONSTANT int := 22587;
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM person_emails
                  WHERE person_id = drop_ AND lower(email) = 'chernandez5000@holyfamily.edu') THEN
    RAISE NOTICE 'person % does not own chernandez5000@holyfamily.edu — skipping', drop_;
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'merged person % into % (chernandez5000@holyfamily.edu + users row now on Christian Hernandez)', drop_, kept;
END $$;
