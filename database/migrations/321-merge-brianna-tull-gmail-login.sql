-- Brianna Tull (person 22466, LA user 57729893) registered with LeagueApps
-- under brianna.tull@icloud.com but signs in to FH with Google as
-- mzsteik10@gmail.com.  Her first Google login (2026-09-01) predates the
-- OAuth name-match path being able to help (Google returned no name
-- claims), so OAuthController minted a blank orphan person (22652) with
-- the gmail + a users row (153) and zero roster ties.
--
-- person_emails.email is UNIQUE across all persons, so the gmail can't be
-- added to 22466 while the orphan still holds it.  Merge the orphan into
...
--
-- Idempotent: no-ops if 22652 is already gone or no longer owns the email.
DO $$
DECLARE
  kept  CONSTANT int := 22466;
  drop_ CONSTANT int := 22652;
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM person_emails
                  WHERE person_id = drop_ AND lower(email) = 'mzsteik10@gmail.com') THEN
    RAISE NOTICE 'person % does not own mzsteik10@gmail.com — skipping', drop_;
    RETURN;
  END IF;
  ...
  --    the persons DELETE can't trip a FK.  (All zero rows as of 2026-09-03.)
  ...
  RAISE NOTICE 'merged person % into % (mzsteik10@gmail.com + users row now on Brianna Tull)', drop_, kept;
END $$;
