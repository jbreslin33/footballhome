-- 346 — Merge Jamie Arevalo's Google-login orphan into her LeagueApps person.
--
-- Jamie Arevalo (person 22193, LA user 58290114) is on the Tri County
-- Women roster and registered under arevalojamie@yahoo.com.  On
-- 2026-09-08 00:07 UTC she signed in with Google, which reported the
-- address as "Arevalojamie@yahoo.com".  OAuthController::findOrCreateUser
-- looked it up with a case-SENSITIVE `email =`, missed her row, and the
-- name fallback could not fire because Google sent no given_name.  Result:
-- a blank orphan (22687, last name only) holding the capitalised email +
-- users row 186, with zero roster ties.  Same split as 321/323/325/327/
-- 344/345, new cause (case).  The lookup is fixed in the same commit.
--
-- Merge the orphan into 22193 exactly the way PersonMerge::merge() does
-- (audit row + reparent + drop), so it is reversible from the person
-- profile like every other GM→LA merge in person_merges.  The capitalised
-- email row is dropped rather than moved: 22193 already owns the same
-- address in lower case and person_emails is UNIQUE(email).
--
-- Idempotent: no-ops if 22687 is already gone or no longer owns the email.
DO $$
DECLARE
  kept  CONSTANT int := 22193;
  drop_ CONSTANT int := 22687;
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM person_emails
                  WHERE person_id = drop_ AND lower(email) = 'arevalojamie@yahoo.com') THEN
    RAISE NOTICE 'person % does not own arevalojamie@yahoo.com — skipping', drop_;
    RETURN;
  END IF;
  ...
  --    (22193 has never had one, so users 186 becomes hers).
  ...
  -- kept already has the same value (case-insensitive for email, since
  -- the global UNIQUE(email) would otherwise let both spellings coexist).
  ...
  -- Google verified the address; carry that onto the kept row's spelling.
  UPDATE person_emails SET is_verified = true, verified_at = COALESCE(verified_at, now())
   WHERE person_id = kept AND lower(email) = 'arevalojamie@yahoo.com' AND NOT is_verified;

  -- 3. Anything left on the dropped person is in the snapshot; clear it so
  --    the persons DELETE can't trip a FK.  (All zero rows as of 2026-09-07
  --    except the capitalised email row, which is dropped on purpose.)
  ...
  RAISE NOTICE 'merged person % into % (users row now on Jamie Arevalo)', drop_, kept;
END $$;
