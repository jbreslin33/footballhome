-- Sergio Rico (person 22615, LA user 58359818) registered with LeagueApps
-- under dieguihno15@hotmail.com but signs in to FH with Google as
-- ricostore07@gmail.com.  His first Google login (2026-08-21) minted a blank
-- orphan person (22590, last name only) holding that email + a users row
...
-- added to 22615 while the orphan still holds it.  Merge the orphan into
...
--
-- Idempotent: no-ops if 22590 is already gone or no longer owns the email.
DO $$
DECLARE
  kept  CONSTANT int := 22615;
  drop_ CONSTANT int := 22590;
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM person_emails
                  WHERE person_id = drop_ AND lower(email) = 'ricostore07@gmail.com') THEN
    RAISE NOTICE 'person % does not own ricostore07@gmail.com — skipping', drop_;
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'merged person % into % (ricostore07@gmail.com + users row now on Sergio Rico)', drop_, kept;
END $$;
