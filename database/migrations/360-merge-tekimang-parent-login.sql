-- 360 — Merge the Tekimang family's login orphan into Cayden's LeagueApps parent.
--
-- Fenny Veronica (person 22443, LA user 57680425) is the parent of
-- Cayden Tekimang (22442), on the U8 Travel roster.  LeagueApps has the
-- parent as f3nny_81@yahoo.com / +16262833793.  On 2026-09-13 06:20 UTC
-- someone in the household signed in as handichen21@yahoo.com — an
-- address LA never had — so OAuthController::findOrCreateUser found no
-- match and created a blank orphan (22702, last name "Tekimang" only)
-- holding that email + users row 205, with no children, roster ties, or
-- LA membership.
--
-- Result: the phone holds a JWT for 22702.  requireSession checks the
-- bearer token before the session cookie, so the SMS/email magic links
-- minted for 22443 (9/9, 9/11, 9/16 — all opened on an iPhone) never
-- took effect — every request still resolved to the empty orphan, hence
-- "Not a member of any club chat" and no schedule (owner 2026-09-17).
-- Same split as 344 (Ellerbee) and 353 (Allen).
--
-- handichen21 may be Cayden's OTHER parent rather than Fenny herself.
-- persons.parent_person_id holds one guardian per child, so either way
-- the household's login has to land on 22443 to see Cayden's events;
-- guardians only read (they never RSVP as themselves), so sharing the
-- parent row is harmless.  Reversible from the person profile like every
-- other merge in person_merges.
--
-- Idempotent: no-ops if 22702 is already gone or no longer owns the email.
DO $$
DECLARE
  kept  CONSTANT int := 22443;
  drop_ CONSTANT int := 22702;
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM person_emails
                  WHERE person_id = drop_ AND lower(email) = 'handichen21@yahoo.com') THEN
    RAISE NOTICE 'person % does not own handichen21@yahoo.com — skipping', drop_;
    RETURN;
  END IF;
  ...
  --    (22443 has never had one, so users 205 becomes theirs).
  ...
  -- kept already has the same value.  22443 owns only f3nny_81@yahoo.com,
  ...
  -- keeps working (0 rows as of 2026-09-17, harmless if so).
  ...
  --    the persons DELETE can't trip a FK.  (All zero rows as of 2026-09-17.)
