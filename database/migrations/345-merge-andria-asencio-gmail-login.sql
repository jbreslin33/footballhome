-- Andria Asencio (person 22515, LA user 57947501) is the LeagueApps parent
-- of Gian Silvestry (22514) and registered under
-- asencio9495@yahoo.com.  She signs in to FH with Google as
-- andria9495@gmail.com.  Her first Google login (2026-09-02) minted a blank
-- orphan person (22663, last name only) holding that email + a users row,
-- with no child link and zero roster ties, so she sees no events.
-- Same "different emails, so auto-adopt can't catch it" split as 321/323/325/327/344.
--
-- person_emails.email is UNIQUE across all persons, so the gmail can't be
-- added to 22515 while the orphan still holds it.  Merge the orphan into
...
--
-- Idempotent: no-ops if 22663 is already gone or no longer owns the email.
DO $$
DECLARE
  kept  CONSTANT int := 22515;
  drop_ CONSTANT int := 22663;
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM person_emails
                  WHERE person_id = drop_ AND lower(email) = 'andria9495@gmail.com') THEN
    RAISE NOTICE 'person % does not own andria9495@gmail.com — skipping', drop_;
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'merged person % into % (andria9495@gmail.com + users row now on Andria Asencio)', drop_, kept;
END $$;
