-- Kareemah Ellerbee (person 22665, LA user 58431456) is the LeagueApps parent
-- of Zane Stone (22664, U16 Intramural) and registered under
-- ellerbeekareemah@yahoo.com.  She signs in to FH with Google as
-- kreemykakes1@gmail.com.  Her first Google login (2026-09-04) minted a blank
-- orphan person (22678, last name only) holding that email + a users row,
-- with no child link and zero roster ties, so she sees no events.
-- Same "different emails, so auto-adopt can't catch it" split as 321/323/325/327.
--
-- person_emails.email is UNIQUE across all persons, so the gmail can't be
-- added to 22665 while the orphan still holds it.  Merge the orphan into
...
--
-- Idempotent: no-ops if 22678 is already gone or no longer owns the email.
DO $$
DECLARE
  kept  CONSTANT int := 22665;
  drop_ CONSTANT int := 22678;
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM person_emails
                  WHERE person_id = drop_ AND lower(email) = 'kreemykakes1@gmail.com') THEN
    RAISE NOTICE 'person % does not own kreemykakes1@gmail.com — skipping', drop_;
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'merged person % into % (kreemykakes1@gmail.com + users row now on Kareemah Ellerbee)', drop_, kept;
END $$;
