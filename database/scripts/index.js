...
async function generateApslSql(conferences, parser) {
  ...
  if (teams.length > 0) {
    ...
    lines.push('');
    lines.push('INSERT INTO teams (id, division_id, name, notes) VALUES');
    
    const valueLines = teams.map((team, idx) => {
      const isLast = idx === teams.length - 1;
      return `  (${team.id}, ${team.division_id}, '${team.name.replace(/'/g, "''")}', '${team.notes}')${isLast ? '' : ','}`;
    });
    
    lines.push(...valueLines);
    lines.push('ON CONFLICT (id) DO UPDATE SET');
    lines.push('  division_id = EXCLUDED.division_id,');
