...
`;
    ...
    const outputPath = path.join(__dirname, 'sql', `100.${this.leagueId}-organizations-apsl.sql`);
...
`;
    ...
    const outputPath = path.join(__dirname, 'sql', `101.${this.leagueId}-clubs-apsl.sql`);
    fs.writeFileSync(outputPath, sql);
    console.log(`   ✓ ${outputPath}`);
  }
...
ON CONFLICT (division_id, name) DO NOTHING;\n`;
    ...
    const outputPath = path.join(__dirname, 'sql', `102.${this.leagueId}-teams-apsl.sql`);
    fs.writeFileSync(outputPath, sql);
    console.log(`   ✓ ${outputPath}`);
  }
