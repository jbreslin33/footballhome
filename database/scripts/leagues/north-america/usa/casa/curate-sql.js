...
class CasaSqlCurator extends BaseSqlCurator {
  ...
  async curate() {
    ...
    // Read APSL SQL files
    const apslConfig = JSON.parse(fs.readFileSync(path.join(__dirname, '../apsl/config.json'), 'utf8'));
    const apslDir = path.join(__dirname, '../apsl/sql');
    ...
    // Read CSL SQL files
    const cslConfig = JSON.parse(fs.readFileSync(path.join(__dirname, '../csl/config.json'), 'utf8'));
    const cslDir = path.join(__dirname, '../csl/sql');
