...
/**
 * CSL Update Orchestrator
 *
 * Diff-based update flow:
 *   1. Load old snapshot (database/scraped-html/csl/snapshot.json)
 *   2. Scrape fresh HTML from cosmosoccerleague.com (standings + team pages)
 ...
 *
 * Usage:
 *   node update-csl.js                   # Scrape + diff + update SQL files
 *   node update-csl.js --dry-run         # Scrape + diff only (no SQL changes)
 *   node update-csl.js --db              # Also run SQL against live database
 *   node update-csl.js --baseline        # Re-parse cached HTML → save snapshot
 *   node update-csl.js --skip-scrape     # Use existing cached HTML (no network)
 */
...
const CACHE_DIR = path.join(__dirname, '../../../../../scraped-html/csl');
const SQL_DIR = path.join(LEAGUE_DIR, 'sql');
const SNAPSHOT_PATH = path.join(CACHE_DIR, 'snapshot.json');

// CSL URL patterns
const STANDINGS_URL = CONFIG.standingsUrl || 'https://www.cosmosoccerleague.com/CSL/Tables/';
const TEAM_PAGE_BASE = 'https://www.cosmosoccerleague.com/CSL/Team';
...
class CslUpdater {
  ...
  /**
   * Scrape fresh HTML from cosmosoccerleague.com
  ...
  async scrapeHtml() {
    ...
    try {
      ...
      const CslSqlGenerator = require('./generate-sql');
      const tempGenerator = new CslSqlGenerator();
  ...
  /**
   * Parse cached HTML into a LeagueSnapshot using the CSL generator
   */
  parseSnapshot() {
    const CslSqlGenerator = require('./generate-sql');
  ...
  async run() {
    ...
    if (!this.skipScrape) {
      console.log('\n🌐 Scraping fresh data from cosmosoccerleague.com...');
...
if (require.main === module) {
  ...
  const updater = new CslUpdater(options);
  updater.run()
    .then(() => process.exit(0))
    .catch(error => {
