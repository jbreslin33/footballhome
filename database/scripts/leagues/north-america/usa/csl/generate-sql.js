...
`;
    ...
    const outputPath = path.join(__dirname, 'sql', `100.${this.leagueId}-organizations-csl.sql`);
    fs.writeFileSync(outputPath, sql);
    console.log(`   ✓ ${outputPath}`);
  }
...
`;
    ...
    const outputPath = path.join(__dirname, 'sql', `101.${this.leagueId}-clubs-csl.sql`);
    fs.writeFileSync(outputPath, sql);
    console.log(`   ✓ ${outputPath}`);
  }
