#!/usr/bin/env node
/**
 * Import GroupMe Old Timers Liga 2 Chat Members to external_identities
 * 
 * Creates external_identities with team_id (Lighthouse Old Timers Club).
 * 
 * Usage: node scripts/import-groupme-old-timers-users.js
 */
...
async function importOldTimersUsers() {
  try {
    await client.connect();
    console.log('🔌 Connected to database');

    // Get Lighthouse Old Timers Club team ID
    ...
    console.log(`📥 Fetching members from Old Timers chat (${OLD_TIMERS_GROUP_ID})...`);
    const group = await apiRequest(`/groups/${OLD_TIMERS_GROUP_ID}`);
    ...
    console.log('');
    console.log('✅ Old Timers users import complete');
