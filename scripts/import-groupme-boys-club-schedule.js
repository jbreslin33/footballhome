#!/usr/bin/env node
/**
 * Import Boys Club Liga 1 Chat Schedule
 * 
 * Imports game schedule from the Lighthouse Boys Club Liga 1 chat.
 * No practices - games only.
 * 
 * Usage: node scripts/import-groupme-boys-club-schedule.js
 */
...
async function importSchedule() {
  try {
    ...
    console.log(`📥 Fetching calendar events from Boys Club chat (${BOYS_CLUB_GROUP_ID})...`);
    
    const events = await apiRequest(`/conversations/${BOYS_CLUB_GROUP_ID}/events/list`);
    ...
    console.log('');
    console.log('✅ Boys Club schedule import complete');
