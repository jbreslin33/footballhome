...
# ============================================================
# Step 3b: Install/Check Google Chrome (needed for scraping)
# ============================================================
...
if [ "$OS_TYPE" == "Darwin" ]; then
    if [ -x "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" ]; then
        CHROME_FOUND=true
        print_success "Google Chrome is installed (macOS)"
    fi
...
if [ "$CHROME_FOUND" = false ]; then
    ...
    if [ "$OS_TYPE" == "Linux" ]; then
        ...
        # Download and install Chrome's signing key and repo
        curl -fsSL https://dl.google.com/linux/linux_signing_key.pub | sudo gpg --dearmor -o /usr/share/keyrings/google-chrome-keyring.gpg 2>/dev/null || true
        ...
        if command -v brew &> /dev/null; then
            ...
            print_warning "Chrome is required for scraping league websites (make scrape-apsl, make scrape-csl)"
        fi
    fi
fi
