...
- [ ] `sudo podman logs footballhome_sim_${match_id}` shows only that match's logs (no interleaving; proven by test with 3 concurrent matches — awaits Slice 14.7 concurrent-match load test).
