...
- 33.4 [x] `AiController::defaultBehaviors()` attacking-role bags locked for ST9/ST10 as the concrete 8-behavior stack `{Feint1v1Behavior, SupportOffBallBehavior, CoverShadowBehavior, CompactShapeBehavior, OverloadSupportBehavior, SwitchSideSupportBehavior, ReceivingUnderPressureBehavior, ThirdManRunBehavior}`. Landed 2026-07-19: `test_ai_controller` asserts the bag size and identity for LCB/RCB/CDM + ST9/ST10; 1v1 attacker slots continue to use `Role::ST9` (no `Role::Forward` enum yet).
...
**Slice 33 exit gate**: coach spawns a scenario with an `AiController` attacker slot on `Role::ST9` and a human defender; the attacker uses the locked bag and feints past the defender when the defender commits early to press.
