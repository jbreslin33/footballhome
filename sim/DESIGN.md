...
- [ ] CI load test asserting zero drops under simulated 100 ms Pg latency for a 10-min match — deferred to the exit-criteria sub-slice below (§16.5 additions).
...
- [ ] `sim/scripts/check_determinism_cross_arch.sh` extended: after native amd64 run, replay the same match from the recorded input log; assert same hash. Then run same replay under arm64 via qemu; assert same hash again. Three-way byte equality (live amd64 = replay amd64 = replay arm64). **Deferred to sub-slice 6.1** — needs `--verify` against a real Postgres, which requires the compose stack to be up during the CI script; separated so the binary itself is reviewable/mergeable independently.
...
- [~] `GET /api/sim/debug/matches/:id/state?tick=T` — **stub landed, real impl deferred to sub-slice 8.1.** Currently returns `HTTP 501 {error:"not implemented", match_id, tick, reason:"fh-sim-replay lives in footballhome_sim, not footballhome_backend — cross-container spawn deferred to sub-slice 8.1", workaround:"run manually inside the sim container: sudo podman exec footballhome_sim fh-sim-replay --match-id N --up-to-tick T --emit-hex"}`. The route registers + auth-gates + rate-limits normally; only the replay spawn is stubbed. **Reason for deferral:** the spec called for spawning `fh-sim-replay` as a sandboxed subprocess of the backend, but the binary is built into the `footballhome_sim` image (not `footballhome_backend`) — spawning across containers is a distinct architectural choice not appropriate to hand-wave inside sub-slice 8. See sub-slice 8.1 below for the two follow-up paths.
...
**Sub-slice 8.1 — Cross-container replay for `/state` (deferred from sub-slice 8)**
- [ ] Path (a): bundle `fh-sim-replay` into the backend image. Adds ~10MB to the backend image (sim dependencies) and couples backend build to sim build; sandbox becomes trivial (setuid/setgid, no network, tmpfs cwd). Preferred if the sim binary stays small and its transitive deps don't pull in a large runtime.
- [ ] Path (b): keep `fh-sim-replay` in the sim container and expose it via a tiny UNIX-socket IPC (`sim/src/net/ReplayDaemon.{h,cpp}`, listens on `/var/run/footballhome/replay.sock` inside the sim container, shared into backend via a `type: bind` volume in docker-compose). Backend sends `{match_id, up_to_tick}` JSON line, sim daemon replies with the JSON snapshot. Keeps image sizes clean, decouples build graphs, but adds an IPC surface to keep audited.
- [ ] Decision blocker: need to measure `fh-sim-replay` transitive deps first — if <5MB extra, prefer path (a); otherwise path (b).
...
- [~] Every match played end-to-end has: (a) a `sim_matches` row with `ended_at` set — done via `updateMatchEnded`; (b) `sim_match_inputs` rows for every accepted input — done via `AsyncPgLog<InputRow>`; (c) `sim_match_events` rows for `MatchStart` + all `ClientConnect`/`Disconnect`/`SlotClaim`/`SlotRelease` + `MatchEnd` with canonical snapshot hash — done via `AsyncPgLog<EventRow>` + `main.cpp` shutdown sequence; (d) `sim_matches.result` populated with the same hash — done. Manual end-to-end verification still pending — no automated test yet asserts the row-set exists after a scripted match.
...
- [ ] Cross-arch determinism CI green with DB-sourced replay: live amd64 = replay amd64 = replay arm64 (via qemu). Blocked on sub-slice 6.1 (extending `check_determinism_cross_arch.sh` to invoke `fh-sim-replay --verify` under qemu-arm64).
...
- [~] Admin `GET /api/sim/debug/matches/:id/state?tick=T` returns the deterministic snapshot at tick T for any recorded match. Route + auth + rate-limit landed in sub-slice 8 (2026-07-13); currently returns HTTP 501 with a workaround. Blocked on sub-slice 8.1 (cross-container `fh-sim-replay` spawn).
