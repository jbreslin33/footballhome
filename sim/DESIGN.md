...
>
> **Slice 32.1 (utility-AI hysteresis core + ADR §22.26 accepted):** landed 2026-07-18. `IBehavior::minTicks()` defaults to 10 ticks; `AiController` tracks the current behavior and start tick, keeps the active behavior through its minimum commitment window when still gated/positive-utility, and applies a deterministic fixed-point switch penalty to non-current challengers after the window. Next up: Slice 32.2 no-oscillation golden.
