...
>
> **Next up:** finish Slice 31 defender behavior shape: full `JockeyBehavior` positioning, role-specific behavior bags, and the first defender determinism goldens. `MarkOpponentBehavior` skeleton + `SlotSpawn::mark_target` plumbing is already in place and validated at 54/54 sim tests.
