...
>
> **Next up:** add the first defender determinism goldens. The defender behavior bag now lives on concrete defensive roles (`LCB`/`RCB`/`CDM`) and `BallOnPitchWithDefenderScenario` routes its AI slot through `Role::LCB`; goal-side/team-aware refinements wait for the next scenario/model slice that exposes those facts in `AwarenessView`.
