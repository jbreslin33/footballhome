...
- [x] `sim/src/persistence/ProfileStore.{hpp,cpp}` — thin wrapper over `IPgClient` that reads/writes `sim_player_profile` bytea via the existing `AttributeSet::toBytes/fromBytes`, `ConceptSet::toBytes/fromBytes`, `RecognitionSet::toBytes/fromBytes` codecs.
- [x] `ProfileStore::loadOrCreate(person_id)`: if row exists → decode + return; else materialize a default `PlayerProfile` from `defaultPhysical()` + `defaultConcepts()`, persist, return. First-touch is idempotent.
...
- [x] Test: `test_profile_store.cpp` — round-trips a profile through `InMemoryPgClient`, verifies byte-exact recovery, verifies default materialization for unknown `person_id`.
...
**Consequences**:
+ Zero write amplification: exactly one INSERT per person, ever, for the entire lifetime of the M0 product. The `updated_at` column stays equal to `created_at` for the whole M0 milestone — a useful smoke check for the invariant (`SELECT COUNT(*) FROM sim_player_profile WHERE updated_at <> created_at` should always return 0 in M0).
...
- **The M0 smoke check fires** — i.e. `SELECT COUNT(*) FROM sim_player_profile WHERE updated_at <> created_at` returns non-zero in an M0 environment. That means the CI check has been bypassed or misses a call path; investigate immediately, do NOT normalize the invariant loss.

**Refs**: §5.7 (Match::end lifecycle — no profile write in M0), §14 (match lifecycle — first-touch INSERT-only policy called out inline), §16.6 (Slice 13 spec — where `ProfileStore` landed), §21.2 item 2 (this ADR resolves that checkbox), §22.4 (bytea persistence — why `upsertProfile` deals in `span<const std::byte>`), §22.12 (persistence architecture — `IPgClient::upsertProfile` contract), §22.13 (replay depends on this policy — the "every DB profile IS the M0 default" invariant is what §22.13 exploits).

---

## 23. Milestone 1 — detailed spec
...
- [ ] [sim/src/controller/DribbleIntent.hpp](sim/src/controller/DribbleIntent.hpp) — extension to the existing `Intent` shape: `direction` (Fixed64 heading), `speed_frac` (Fixed64 in [0,1] scaling player's max_walk_speed).
- [ ] [sim/src/controller/HumanController.cpp](sim/src/controller/HumanController.cpp) `decide()` emits `DribbleIntent` when the player is within `ball_control_radius` (Fixed64 constant, ~0.5 m) of the ball AND the ball's `owner` is either `nullopt` (first-touch pickup) or the player's own `SlotId` (continuing to dribble).
- [ ] [sim/src/mechanics/BallControl.{hpp,cpp}](sim/src/mechanics/BallControl.hpp) — new mechanic resolving player-ball interaction each tick: ball follows the owning player at `heading + small_offset` (Fixed64), ball speed capped at `player.mechanics.max_walk_speed * dribble_efficiency` (`dribble_efficiency` = new `physical` attribute, Fixed64 in [0,1], default 0.85 via `m0::defaultPhysical()` extension — see ADR §22.15 draft below).
- [ ] `Match::tick` integrates: read `WorldView`, run `HumanController::decide` per slot, apply `BallControl` before physics step, integrate ball, integrate players, snapshot for wire.
...
- 16.5 Client-side: joystick input unchanged; renderer draws a small indicator ring around the player currently owning the ball.
- 16.6 Determinism test: fixed inputs → identical player+ball trajectory across arches; two-player first-touch test (both close on ball) → same winner + same subsequent trajectory across arches.

**Slice 16 exit gate**: connecting one browser client to a `BallOnPitchScenario` match lets the user dribble the ball around the pitch with the joystick. Two clients competing for the same ball resolves deterministically (first-touch wins by rule 16.3).
...
- 17.5 `SoftDrillScenario` (new) — uses `Soft` mode on a rectangle inside the pitch to demo the pushback feel for the "coach can wander outside the drill zone but is bounced back" UX.
- 17.6 Tests: `test_playable_area_hard.cpp` (clamp + velocity-zeroing), `test_playable_area_soft.cpp` (force magnitude matches `k × penetration_depth`), determinism cross-arch for both.
...
- 18.1 New frontend hub tile "Dribble Drill (M1)" on [frontend/tactical-games.html](frontend/tactical-games.html) — spawns a fresh `HalfPitchScenario` match via `SimOrchestrator`, redirects the browser to `/sim/${match_id}`.
- 18.2 M1 exit-criteria checklist (§23.4 below) all green.
...
- 18.4 CI: cross-arch determinism includes M1 dribble scenario; load test covers 20 concurrent M1 matches.
- 18.5 Backfill any `§21.X` M2-blocker items uncovered during M1 dev into a new §21.7 or §21.8 subsection.
...
- [ ] A player can dribble a ball across an empty pitch (Slice 16).
- [ ] Two players cannot both control the same ball — first-touch wins, tie broken deterministically by lower `SlotId` (Slice 16.3).
- [ ] A `HalfPitchScenario` with `Hard` playable-area prevents players from crossing the halfway line (Slice 17.4).
- [ ] `Soft` playable-area smoothly pushes players back on penetration (Slice 17.5).
- [ ] `SimOrchestrator` launches a fresh match container in < 2 s cold-start, < 500 ms warm-image start (Slice 14.7 load test asserts both).
...
- [ ] **Cross-match input isolation invariant.** Slice 14.7's load test needs to verify not just "no dropped inputs" but "no input from match A ever appears in match B's `sim_match_inputs`" — either by construction (each daemon has its own `AsyncPgLog<InputRow>` per §22.12) or by a post-test SQL query.
- [ ] **AsyncPgLog per-daemon vs pooled decision revisit** (§22.12 revisit condition). If Slice 14.7 shows 3N connections becoming a Postgres `max_connections` pressure, evaluate pgbouncer between sim daemons and Postgres (transaction-pooling mode; prepared statements are per-connection, `PgClient` re-declares them, so libpqxx + pgbouncer coexist transparently).
