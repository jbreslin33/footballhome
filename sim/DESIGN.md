...
>
> **Next up:** finish Slice 31 defender behavior shape with role-specific behavior bags and the first defender determinism goldens. `JockeyBehavior` now has velocity-aware lane positioning, while goal-side/team-aware refinements wait for the next scenario/model slice that exposes those facts in `AwarenessView`.
