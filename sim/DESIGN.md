...
- [ ] All §24.4 exit-criteria boxes ticked.
- [ ] Cross-arch determinism CI green for every new M2 golden (`sim/scripts/check_determinism_cross_arch.sh` unchanged; it runs whatever `test_determinism` binary emits).
- [ ] M3 blocker sweep: any new tech debt discovered during M2 catalogued into §21.8 with revisit-conditions before M3 starts.
- [ ] `sim/DESIGN.md` §15 milestone table flipped M2 → **done**; §21.8 M3-blockers section appended if any items caught during the sweep.
...
- [ ] **26.6 — Determinism goldens for the pass primitive** (only remaining item to close Slice 26). Two new `FH_TEST`s appended to `sim/tests/test_determinism.cpp`, following the exact pattern of the existing 9 goldens (`Wander200`, `HumanSprint400`, `BallRoll400`, `Dribble200`, `FirstTouch200`, `HalfPitchHard400`, `SoftDrill400`, `BallOnPitchWithDefender400`, plus the M0 canonical hash):
    1. **`pass_east_slot1_to_slot2_400_ticks_seed_42`** — reuses `BallAndTwoSlotsScenario` (already defined at the top of `test_determinism.cpp`). Spawn: ball at origin at rest; SlotId{1} at (-0.3, 0, 0) with heading east; SlotId{2} at (+15, 0, 0) with heading west. Claim slot 1 with a `HumanController`. Slot 2 is intentionally unclaimed → IdleController (no `wants_dribble`, so Rule 1 first-touch on the receiving side does NOT fire — the golden captures the *pass flight + coast-to-rest* trajectory alone, not the receive). Input on slot 1: tick 1 asserts `wants_dribble=true` + `desired_direction=(+1, 0, 0)` so first-touch grabs the ball; tick 10 asserts `wants_kick=true` + `kick_direction=(+1, 0, 0)` + `kick_power_hint=0` (falls through to `MechanicsParams::pass_power = 15 m/s`). Run 400 ticks (20 s). Fingerprints: release-on-kick timing, `applyImpulse` speed math, `kKickAliveTicks=40` friction suppression window, post-window rest-band snap-to-zero, plus the whole `wants_kick` INPUT path from `Match::applyInput` through `BallControl` to `BallPhysics`.
    2. **`pass_receive_first_touch_200_ticks_seed_42`** — same `BallAndTwoSlotsScenario` layout but with SlotId{2} ALSO claimed by a `HumanController` and continuously asserting `wants_dribble=true` + `desired_direction=(-1, 0, 0)` (walking west toward the incoming ball). Slot 1 fires the same tick-1 dribble + tick-10 kick script. Somewhere around tick ~30–50 the ball crosses within `kBallControlRadius = 0.5 m` of slot 2's position → Rule 1 first-touch transfers ownership; from then on the ball glues to slot 2 and both continue west. Run 200 ticks (10 s). Fingerprints: full pass-and-receive handoff — release-on-kick, in-flight travel, Rule 1 pickup at the receiver, Rule 3 glue on the new owner. Locks the two-human M2 exit path into the CI gate.
    3. **Recording protocol**: same as every previous golden per `test_determinism.cpp` header comment. Write the test with a placeholder `constexpr std::uint64_t kExpectedHashPassEast400 = 0x0ULL;`. Podman-build the image (`sudo podman build -f sim/Dockerfile -t footballhome_sim:test-26-6 sim/`); ctest will fail on the placeholder and print the *actual* hash from the drift-message stderr. Paste that hash back into the constant and rebuild — 47+2 = 49 tests green. Commit the source + the hash in the same commit. Cross-arch verification via `sim/scripts/check_determinism_cross_arch.sh` (unchanged).
    4. **Non-goals for 26.6**: no gameplay changes, no new attributes, no schema migration, no wire change. Pure test-only. If your golden fails, something *else* moved — do not update the hash without understanding what drifted.
- ~~26.6 Determinism goldens: `pass_east_slot1_to_slot2_400_ticks_seed_42`, `pass_receive_first_touch_200_ticks_seed_42`.~~ *(duplicate line — folded into the detailed spec above 2026-07-16 as part of the lbreslin handoff)*

**Slice 26 exit gate**: two coaches on two devices open `BallOnPitch2v0`, one claims slot 1 + one claims slot 2, they can pass the ball back and forth with a kick button. Each pass shows a motion trail on both clients. Determinism-locked. **Two-human pass + trail closed 2026-07-16 in Slice 26.4 + 26.5. Only Slice 26.6 goldens remain to fully close.** Full handoff spec (files, test bodies, recording protocol, verification steps) at [docs/handoff-sim-slice-26-6-goldens.md](../docs/handoff-sim-slice-26-6-goldens.md).
...
**Slice 29 — M2 close-out** (not started)

- 29.1 §24.4 exit-criteria sweep — all boxes ticked.
- 29.2 §24.5 non-goals sanity check — nothing accidentally landed inside M2 that should be M3+.
- 29.3 §21.8 M3-blockers section append with any tech debt caught during M2.
- 29.4 §15 milestone flip: M2 → done.

**Slice 29 exit gate**: §24.4 all `[x]`, cross-arch CI green, DESIGN.md updated to reflect close-out.
