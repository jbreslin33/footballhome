...
- [ ] `BallOnPitch2v0Scenario` — two claimable human slots, no defender. First scenario that actually needs the pass primitive (spec'd in Slice 26.3).
...
- [ ] **Wire v1.2 vs additive v1.1 extension.** Slice 26.2 adds `wants_kick + direction` to the INPUT frame. v1.1's snapshot trailer added ball state via length-prefixed extensible region (ADR §22.20). Input frames don't have that mechanism today. Decision: (a) extend INPUT frame with a length-prefixed extension region mirroring §22.20, (b) bump wire to v1.2 with a fixed-layout new INPUT variant, (c) reserve the direction bytes in existing INPUT and version-gate them on capability bit. Recommend (a) — matches the SNAPSHOT precedent, keeps v1 clients working. Land as ADR §22.23 if we pick option (a), §22.24 if we pick (c).
