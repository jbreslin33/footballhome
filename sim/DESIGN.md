...
- [x] Verified end-to-end 2026-07-13: from inside `footballhome_backend`, direct probes to `http://footballhome_sim:9101/admin/replay` returned 401/403/404/500 correctly per bearer/match state. From the host with a minted admin JWT hitting `/api/sim/debug/matches/1/state`, the sim's 500 (`replayMatch: unknown scenario_id=1 (M0 supports only scenario_id=0 / EmptyPitchScenario)`) is forwarded verbatim — proving both hops (backend auth + RPC + sim admin + replay) end-to-end. The scenario_id=1 in the DB is a pre-existing data condition (from an earlier sim run before `upsertMatch`'s `ON CONFLICT DO UPDATE SET server_version = EXCLUDED.server_version` clause was clarified to leave scenario_id sticky), not an RPC bug — a fresh match_id would 200 through the full path.
