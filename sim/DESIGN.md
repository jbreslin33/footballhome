...
- 30.2 [ ] `PursueBallCarrierBehavior` + migration 224 (`pressing` concept id=3) + `Scenario::applyConceptOverrides` hook + `AiController::defaultBehaviors(Role::Any)` wired to `{PursueBallCarrierBehavior}` + `DefenderController.{hpp,cpp}` deleted + `Match.cpp` `Defender`-kind dispatch swapped to `AiController` + `BallOnPitchWithDefenderScenario` gets pressing overlay for slot 2. Utility is constant `Fixed64::one()` for Slice 30.2 (single-behavior bag makes magnitude unobservable; `1/distance` shape deferred to Slice 31.4 when `JockeyBehavior` first competes). `execute()` replicates all four `DefenderController::decide()` branches byte-for-byte at intent level (owner-hold, no-ball, ball-missing, on-ball, else-pursue). `BallOnPitchWithDefender400` golden **may** rotate — document new hash at landing (or note stability if unchanged). All 10 other goldens stay byte-identical (no path touches `AiController`). Closes §24.6 DefenderController migration item. New unit test file `test_pursue_ball_carrier_behavior.cpp` (~6 cases) brings ctest total to 66.
...
- 31.1 [ ] Migration 225 (`jockey` id=5, `marking` id=4).
- 31.2 [ ] `JockeyBehavior` + `MarkOpponentBehavior` implementations. `SlotSpawn::mark_target` field added.
- 31.3 [ ] Migration 226 nine-attribute M3 batch (`marking_technique` id=16, `standing_tackle` id=17, `interception` id=18, `feint` id=19, `first_time_pass` id=20, `aggression` id=21, `positioning_sense` id=22, `composure` id=23, `anticipation` id=24). Five of the nine (`marking_technique`, `standing_tackle`, `interception`, `positioning_sense`, `composure`) consumed by `JockeyBehavior::utility()` + `MarkOpponentBehavior::utility()`; the other four (`feint`, `first_time_pass`, `aggression`, `anticipation`) sit dormant until Slice 33's attacker behaviors + Slice 32's hysteresis knobs consume them.
