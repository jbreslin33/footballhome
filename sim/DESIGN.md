...
>
> **Next up (Slice 31.1):** migration 225 seeds `marking` (id=4) + `jockey` (id=5) concepts, both category `defensive`. Doc-only + SQL — no C++ compile impact, all goldens stay byte-identical. Follow migration 224 as the template: guarded setval, self-audit `DO $$ … RAISE EXCEPTION IF NOT EXISTS`, `ON CONFLICT (key) DO NOTHING`. Also append `225-sim-concept-marking-jockey.sql` to `SIM_MIGRATION_EXTRA_FILES` in [sim/CMakeLists.txt](sim/CMakeLists.txt) so the awk codegen picks it up. Apply with `sudo ./database/migrations/run-migrations.sh`, verify with `sudo make sim-deploy` (52/52 ctest + registry-consistency probe still green). See §25.3 Slice 31.1 for the full deliverable list.
...
- 31.1 [ ] **Migration 225 (`jockey` id=5, `marking` id=4).** First actionable slice for a new contributor picking up sim after Slice 30.2. Doc-only + SQL work — no C++ code paths change, no golden hash rotates, no new tests required beyond registry-loader sanity. Template: [database/migrations/224-sim-concept-pressing.sql](database/migrations/224-sim-concept-pressing.sql). Concrete steps:
