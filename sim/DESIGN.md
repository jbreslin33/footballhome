...
### 22.27 [2026-07-27] Revert off-ball/formation behaviors from the ST9/ST10 bag — Slice 33.4 scope correction

Status:       accepted
Context:      Slice 33.4 landed correctly on 2026-07-19 (`c729a2a7`) with the ST9/ST10 attacker bag locked to exactly `{Feint1v1Behavior}`, matching §5.5's explicit reservation of `CoverShadowBehavior` for M5 and §25.5's M3 non-goal "off-ball intelligence (M4 — 2v1/2v2 supporting runs, cover-shadow, press-partner switching, `formation_shape` concept, ...)". On 2026-07-23, commit `4164c67a` ("Add simulator behavior progress and empty-state UI") implemented seven off-ball/formation behaviors (`SupportOffBallBehavior`, `CoverShadowBehavior`, `CompactShapeBehavior`, `OverloadSupportBehavior`, `SwitchSideSupportBehavior`, `ReceivingUnderPressureBehavior`, `ThirdManRunBehavior`) and wired all of them into `AiController::defaultBehaviors()` for `Role::ST9`/`Role::ST10`, gated on `return_to_base`/`stay_in_zone` (migration 232) — concepts §25.3 explicitly documents as having "no consumer in M3." On 2026-07-26, commit `03944756` rewrote this doc's Slice 33.4 bullet to describe the resulting 8-behavior bag as the intended landing, without updating §5.5's M5 reservation or §25.5's non-goals to match — silently expanding M3's scope after §25.4's exit criteria and §15's milestone-done flip had already been ticked. §22.0 is explicit that history is never rewritten to match code after the fact; a scope change like this needs its own ADR, not a retroactive edit to an already-closed slice's bullet.
Decision:     Un-wire the seven off-ball/formation behaviors from `AiController::defaultBehaviors(Role::ST9 | Role::ST10)`, restoring the bag to `{Feint1v1Behavior}` as Slice 33.4 originally (and correctly) locked it. The seven behavior implementations and their standalone unit tests stay in the tree (`sim/src/behavior/*.{hpp,cpp}`, `sim/tests/test_*_behavior.cpp`) as real, tested, but currently-unwired M4 candidate work — `check_behavior_registration.sh` stays green because `AiController.cpp` names them in an explanatory comment at the switch site, which the script's registration check treats as a valid (intentionally coarse) registration site. `sim/DESIGN.md` §25.3 Slice 33.4 bullet and Slice 33 exit gate revert to describing the single-behavior bag; `test_ai_controller.cpp`'s `default_behaviors_slice_33_4_roles_get_correct_bags` reverts to asserting bag size 1.
Consequences: + M3's non-goals and §5.5's M5 reservation are true statements again; the milestone-done flip in §15/§25.4 is no longer contradicted by live code. + Seven behaviors' worth of real implementation and test coverage isn't discarded — it's available for whichever M4 slice properly plans the `return_to_base`/`stay_in_zone` consumer, gives them formation/team context, and re-litigates whether `CoverShadowBehavior` still belongs to M4 or should stay M5 per §5.5. − The revert itself isn't free: two days of attacker-bag behavior (2026-07-23 through this ADR) that shipped to any running scenario briefly included ungated M4 behaviors; no production `sim_match_events`/determinism goldens reference ST9/ST10 off-ball behavior ids, so no golden rotation was needed.
Revisit if:   an M4 slice is properly planned (ADR + non-goals update + migration for any missing gating concepts) to re-introduce some or all of these seven behaviors into the attacker bag — at that point this ADR's "unwired" state is superseded by the M4 slice's own ADR, not by silently re-editing this one.
Refs:         §5.5 (M5 reservation for `CoverShadowBehavior`), §22.0 (append-only / never-retcon-history rule this ADR is itself an instance of following), §25.3 Slice 33.4 (the corrected bullet), §25.4 (M3 exit criteria — unaffected, all criteria were already satisfied by the correct `{Feint1v1Behavior}` bag), §25.5 (M3 non-goals this correction restores), §25.6 (M3→M4 transition prerequisites — the seven unwired behaviors are now a concrete input to that list).
...
AI context routing for simulator concepts and slices lives in
[docs/aider/sim.md](../docs/aider/sim.md). Keep file packs there, not in this
roadmap, so this design stays focused on behavior and status.
...
- New ADRs land at the next available integer in chronological order — never re-numbered per §22.0. §22.26 remains **reserved but not yet drafted** for the utility-AI hysteresis / oscillation-mitigation decision (§21.4 non-standard choice explicitly says "add to M3 spec"; §25.2/§25.3 Slice 32 both mark it "accepted" without a §22 entry existing yet — outstanding doc debt, not part of this correction). §22.27 landed 2026-07-27 (Slice 33.4 scope correction — see §22.27). **Next available for a new ADR is §22.28.**
- SQL migrations continue at **224+**. Slot 223 was sniped between when this section was drafted (2026-07-17) and the first Slice 30 code work by unrelated parallel OOP-LA-sync work (`223-external-aliases-drop-name-uniqueness.sql`) — a mild reminder that migration-slot reservations are advisory, not exclusive; check `database/migrations/` immediately before writing a new SQL file. Revised schedule: 224 = `pressing` concept id=3 (Slice 30.2), 225 = `marking`+`jockey` (Slice 31.1), 226 = M3 attribute batch (Slice 31.3), 229 = `1v1_beat` (Slice 33.1; 227 and 228 were taken by non-sim migrations), 230 = `pattern_being_beaten_1v1` (Slice 33.3), 231 = `OneVsOneAttackDefendScenario` (Slice 34.1), 232 = `return_to_base`+`stay_in_zone` (Slice 34.4). `sim_match_checkpoints` migration slot deliberately **unassigned** — reserved for whichever slice number actually lands the code (see §21.3 item 1 deferral rationale).
...
- [x] Migration 229 (`229-sim-concept-1v1-beat.sql`) — `1v1_beat` id=6, category `on_ball`. Landed 2026-07-18: `sudo make sim-deploy` passed 55/55 ctests, `test_determinism`, and live DB registry consistency. (Slice 33.1)
- [x] Migration 230 (`230-sim-pattern-being-beaten-1v1.sql`) — first `sim_pattern_registry` entry, pattern_id=1. Landed 2026-07-19: `RecognitionSystem` now owns per-carrier recent velocity history and can surface `pattern_being_beaten_1v1` into `AwarenessView::recognized_patterns`; live `Match` receives the boot-loaded `PatternRegistry`. Validated by `test_recognition_passthrough`, `test_registry_loader`, and `sudo make sim-deploy`. (Slice 33.3)
- [ ] Migration 231 (`231-sim-scenarios-1v1-attack-defend.sql`) — `OneVsOneAttackDefendScenario` scenario_id=7. (Slice 34.1)
- [x] Migration 232 (`232-sim-concept-positioning.sql`) — `return_to_base` id=7, `stay_in_zone` id=8, both category `positioning`. Wired but no consumer in M3 (positioning behaviors gated on these lands with M4 formation shape work); ships now so the concept catalog is complete for the milestone the M3 attribute additions target. Landed 2026-07-19 (Slice 34.4). **Note (2026-07-27, ADR §22.27)**: briefly gained a premature M3 consumer (seven off-ball behaviors wired into the ST9/ST10 bag by commit `4164c67a`); reverted, so "no consumer in M3" holds true again pending a properly-planned M4 slice.
...
- [x] Attribute-fed behavior utility plumbing — `AiController` owns the slot's technical + mental `AttributeSet`s and passes them into `IBehavior::utility()`. `JockeyBehavior::utility()` now consumes generated IDs `kPositioningSense` + `kComposure` with deterministic neutral defaults, validated by `test_jockey_behavior`, `sudo make sim-deploy` (`53/53` ctests), and the live registry probe.
- [x] `JockeyBehavior` first positioning pass — `execute()` targets a deterministic 2 m cushion on the defender's current side of the ball carrier and keeps `wants_dribble = false`, `wants_press = false`. This gives jockey a visible back-away/hold-position posture before team goals and role-specific assignments exist.
- [x] `JockeyBehavior` velocity-aware positioning pass — `utility()` now abstains when self/carrier positions are unavailable or a visible loose ball is closer than the carrier, then scores from `positioning_sense` + `composure`. `execute()` targets a deterministic 2 m lane behind the carrier's current velocity vector and falls back to the existing carrier-to-defender cushion when the carrier is stationary. Keeps `wants_dribble = false`, `wants_press = false`. Validated by `test_jockey_behavior` and `sudo make sim-deploy` (`54/54` ctests + registry probe).
- [ ] `JockeyBehavior` goal-side/team-aware positioning — once `AwarenessView` exposes team/goal context, `utility()` scores higher when: (a) self is closest defender to ball, (b) ball_owner != self.team, (c) ball_carrier is heading toward self.goal. `execute()` produces `Intent{ desired_direction = tangent_to_ball_carrier_path, wants_dribble = false, wants_press = false }` — position between ball-carrier and goal, don't commit to the tackle.
- [x] `MarkOpponentBehavior` skeleton — requires `marking` (concept id=4), registers in `AiController::defaultBehaviors(Role::Any)`, abstains without an assigned `mark_target`, scores from `technical.marking_technique` + `mental.positioning_sense`, and executes deterministic non-press/non-dribble movement toward the assigned target. First introduction of a *slot pairing* concept: `AiController` now owns `mark_target: std::optional<SlotId>` sourced from scenario spawn via `SlotSpawn::mark_target` and preserved on runtime `Slot`. Validated by `test_mark_opponent_behavior`, `test_ai_controller`, behavior-registration guard, and `sudo make sim-deploy` (`54/54` ctests + registry probe). Full goal-side shadowing remains open for the defender goldens.
...
- [x] **ADR §22.26** (**drafted 2026-07-17**, accepted 2026-07-18 at Slice 32.1 landing) — utility-AI oscillation mitigation. Closes §21.4 non-standard-choice item 4. Two-part mitigation: (a) minimum-commitment durations per behavior (`IBehavior::minTicks()` — default 10 ticks / 0.5 s), (b) time-since-last-switch penalty (`AiController::decide()` subtracts a decaying bonus from all non-current behaviors, decaying deterministically with a fixed-point 19/20 per-tick multiplier from an initial `kHysteresisBonus = 1/8`). Rationale for both instead of one: minTicks handles rapid oscillation between adjacent-utility behaviors (jockey ↔ press), the switch-penalty handles gradual drift where utility scores converge slowly over many ticks. Chosen over pure minTicks because minTicks can trap a behavior mid-execution when circumstances materially change (e.g. ball_owner flips team); the penalty gives dampening without absolute lockout. Consequences (+) coach-visible AI feels less jittery, (−) tuning knob per behavior means M3 spec now carries per-behavior constants that must survive determinism goldens; migration path for future behaviors: default minTicks = 10 unless overridden.
- [x] `IBehavior::minTicks()` interface method (default 10). `AiController` tracks `current_behavior_started_at: TickNum` and short-circuits `decide()` to keep the current behavior if `now - started_at < current->minTicks()` and the current behavior is still gated with positive utility.
- [x] `AiController::decide()` gains a switch-penalty pass — deducts a deterministic decaying penalty from every non-current behavior's `utility()` return before argmax.
- [x] Determinism golden: `no_oscillation_between_jockey_and_press_400_ticks_seed_42` — direct `AiController` trace fixture with two test-only behaviors whose near-equal utilities alternate every tick (`0.505` vs `0.495`). Locks behavior-transition dampening at 7 transitions over 400 ticks (bound ≤ 8) and golden hash `0xcebce8b37113b617`, validated by `sudo make sim-deploy` (`55/55` ctests + registry probe).

**1v1 attack-vs-defend scenario (Slice 34)**

- [ ] `sim/src/scenario/OneVsOneAttackDefendScenario.{hpp,cpp}` — scenario id=7 (next available; migration 229 registers). Two slots: attacker at (-15, 0) with `Role::ST9` + `ai_profile_source` = configurable person_id (per §21.2 item 1 the `ai_profile_source` field wiring lands here — first consumer), defender at (+10, 0) with `Role::LCB` + `AiController(defaultBehaviors())`. Ball at (-15, 0) glued to attacker at spawn. Goal region at east end from `GoalDrillScenario`. Success: attacker crosses ball into east goal region within 20 s. Reset: 20 s elapsed OR defender owns ball for > 3 s. (Slice 34.1)
...
- [x] `SlotSpawn::mark_target: std::optional<SlotId>` — field added for `MarkOpponentBehavior` to consume when scenario-set and copied into runtime `Slot::mark_target`; `Match` passes it into both initial-spawn and released-slot `AiController` construction paths. Reserved-empty in `OneVsOneAttackDefendScenario` (1v1 has no marking assignment); ships now for the class-layout stability convention (§22 "reserved fields carry their eventual meaning").
...
- 32.1 [x] ADR §22.26 drafted → accepted. `IBehavior::minTicks()` interface method (default 10). `AiController::decide()` learns switch-penalty + minTicks lockout. Landed 2026-07-18; validated by `sudo make sim-deploy` (`55/55` ctests + registry probe).
- 32.2 [x] Determinism golden: `no_oscillation_between_jockey_and_press_400_ticks_seed_42` — assert behavior_transitions ≤ 8 over 400 ticks in a deliberately near-equal-utility fixture. Landed 2026-07-18 at `0xcebce8b37113b617` with 7 transitions; validated by `sudo make sim-deploy` (`55/55` ctests + registry probe).
- 32.3 [x] Slice 31 goldens re-verified byte-identical (hysteresis doesn't kick in when utility spread > `kHysteresisBonus` — deliberate design). Landed 2026-07-18: `sudo make sim-deploy` passed 55/55 ctests, `test_determinism`, and live DB registry consistency.
...
- 33.4 [x] `AiController::defaultBehaviors()` attacking-role bags locked for ST9/ST10 as `{Feint1v1Behavior}` (future shoot/pass stay commented placeholders). Landed 2026-07-19 (`c729a2a7`): `test_ai_controller` asserts bag identity for LCB/RCB/CDM + ST9/ST10; 1v1 attacker slots continue to use `Role::ST9` (no `Role::Forward` enum yet). **Correction 2026-07-27 (ADR §22.27)**: commit `4164c67a` (2026-07-23) had wired seven M4/M5-scoped off-ball/formation behaviors into this bag, and a later doc edit (`03944756`) mis-described that as the intended Slice 33.4 landing; both are reverted. The seven behaviors remain implemented and unit-tested under `sim/src/behavior/` for M4 to wire up properly — see §22.27 and §25.6.
...
- **Seven off-ball/formation behaviors implemented but unwired.** `SupportOffBallBehavior`, `CoverShadowBehavior`, `CompactShapeBehavior`, `OverloadSupportBehavior`, `SwitchSideSupportBehavior`, `ReceivingUnderPressureBehavior`, `ThirdManRunBehavior` exist under `sim/src/behavior/` with standalone unit tests, but are not referenced by `AiController::defaultBehaviors()` for any role (see ADR §22.27 — briefly wired into the ST9/ST10 bag ahead of schedule, reverted). M4 should treat these as a running start rather than building from scratch: plan a real consumer for `return_to_base`/`stay_in_zone` (migration 232, currently unconsumed), give the behaviors team/formation context via `AwarenessView`, and re-decide per §5.5 whether `CoverShadowBehavior` ships in M4 alongside the others or stays M5-only as originally reserved.
