// footballhome sim - OverloadSupportBehavior tests
...
using fh::sim::behavior::OverloadSupportBehavior;
...
FH_TEST(utility_is_positive_when_a_teammate_and_defender_create_an_overload)
{
    OverloadSupportBehavior b;
    ...
    const auto score = b.utility(v, SlotId{2}, overloadConcepts(), emptyAttrs(), emptyAttrs(), std::nullopt);
    FH_EXPECT(score > Fixed64::zero());
}

FH_TEST(execute_moves_to_the_open_side_of_the_overload)
{
    OverloadSupportBehavior b;
    const auto v = makeView(Vec3{Fixed64::zero(), Fixed64::zero(), Fixed64::zero()},
                            Vec3{Fixed64::fromInt(4), Fixed64::zero(), Fixed64::zero()},
    ...
    const Intent intent = b.execute(v, SlotId{2}, overloadConcepts());
