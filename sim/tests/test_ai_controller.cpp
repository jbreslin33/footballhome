...
FH_TEST(default_behaviors_slice_33_4_roles_get_correct_bags)
{
    ...
    // attacker roles (ST9, ST10) get 1 behavior (feint_1v1), and Role::Any,
    // GK remain empty. (No Role::Forward enum yet — 1v1 attacker slots use
    // ST9; see OneVsOneAttackDefendScenario.) The off-ball/formation
    // behaviors (support_off_ball, cover_shadow, compact_shape,
    // overload_support, switch_side_support, receiving_under_pressure,
    // third_man_run) are implemented but explicitly out of scope until M4 —
    // see §22.27 — and must not appear in the ST9/ST10 bag.
