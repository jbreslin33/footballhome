...
class SocialPostCard {
  ...
  async generateImage() {
    ...
    switch (this.postTypeName) {
      case 'game_day':
        ...
        leagueBadgeHtml = this.buildLeagueBadge(league, isCasa);
        break;
      case 'lineup':
    ...
    wrapper.innerHTML = `
      ...
      ">
        <!-- Gold accent bar -->
        <div style="position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(90deg,#f5d442,#ffffff,#f5d442);"></div>

        <!-- Header -->
        <div style="font-size:12px;text-transform:uppercase;letter-spacing:5px;color:#f5d442;margin-bottom:${hasRoster ? '10px' : leagueBadgeHtml ? '10px' : '24px'};font-weight:700;">
        ...
        <div style="margin-top:auto;padding-top:10px;">
          <span style="font-size:10px;letter-spacing:2px;color:rgba(255,255,255,0.3);text-transform:uppercase;">⚽ lighthouse1893sc</span>
        </div>
      </div>
    `;
  ...
  buildImageMatchup(homeName, awayName, dateStr, timeStr, venue, homeLogo, awayLogo) {
    const homeLogoHtml = homeLogo
      ? `<img src="${this.escapeHtml(homeLogo)}" alt="" style="width:56px;height:56px;object-fit:contain;">`
      : `<span style="font-size:2em;">⚽</span>`;
    const awayLogoHtml = awayLogo
      ? `<img src="${this.escapeHtml(awayLogo)}" alt="" style="width:56px;height:56px;object-fit:contain;">`
      : `<span style="font-size:2em;">⚽</span>`;
    return `
      <div style="display:flex;align-items:center;justify-content:center;gap:16px;margin-bottom:20px;width:100%;">
        <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:8px;">
          <div style="width:72px;height:72px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.12);border-radius:12px;border:1px solid rgba(255,255,255,0.18);">${homeLogoHtml}</div>
        ...
        <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:8px;">
          <div style="width:72px;height:72px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.12);border-radius:12px;border:1px solid rgba(255,255,255,0.18);">${awayLogoHtml}</div>
    ...
    return `
      <div style="display:flex;align-items:center;justify-content:center;gap:20px;margin-bottom:20px;width:100%;">
        ...
        <div style="flex-shrink:0;text-align:center;">
          <div style="font-size:48px;font-weight:800;letter-spacing:4px;color:#f5d442;">${this.escapeHtml(String(hs))} - ${this.escapeHtml(String(as))}</div>
        </div>
        <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:8px;">
          <div style="width:72px;height:72px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.12);border-radius:12px;border:1px solid rgba(255,255,255,0.18);">${awayLogoHtml}</div>
          <div style="font-size:13px;font-weight:700;max-width:130px;line-height:1.2;text-transform:uppercase;letter-spacing:0.5px;">${this.escapeHtml(awayName)}</div>
  ...
  buildLeagueBadge(league, isCasa) {
    ...
    return `
      <div style="display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:16px;">
        ...
        ">
          <span style="font-size:14px;font-weight:800;letter-spacing:2px;color:#f5d442;">${this.escapeHtml(league)}</span>
        </div>
      </div>
    `;
  }
  ...
  titleCase(str) {
    ...
    return str.replace(/\b\w+/g, w => {
      if (w.length <= 2 && /^(of|or|in|at|to|by|an|a)$/i.test(w)) return w.toLowerCase();
  ...
  buildImageRoster() {
    ...
    const rows = selected.map(p => {
      const jersey = p.jerseyNumber ? `<span style="color:#f5d442;font-weight:700;font-size:0.9em;min-width:24px;display:inline-block;">#${p.jerseyNumber}</span>` : '';
      const gk = p.isKeeper ? ' <span style="font-size:0.7em;background:rgba(245,212,66,0.25);color:#f5d442;padding:0 4px;border-radius:3px;font-weight:700;">GK</span>' : '';
    ...
    return `
      ...
      <div style="font-size:10px;text-transform:uppercase;letter-spacing:3px;color:#f5d442;margin-bottom:8px;font-weight:700;">SQUAD</div>
