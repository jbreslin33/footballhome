...
class MensRosterScreen extends RosterScreenBase {
  ...
  renderColumn(col, data) {
    ...
    return `
      <div style="background:var(--bg-secondary); border-radius:var(--radius-md); padding:6px; border-top:3px solid #475569; min-width:${this.colBoxMinWidth()};">
        <div style="display:flex; justify-content:space-between; align-items:baseline; flex-wrap:wrap; margin-bottom:4px; gap:6px;">
  ...
  renderPlayer(p, columns, col, position, totalInColumn = 0) {
    ...
    // Roster Role + Official Roster Status dropdowns (2026-08-22) — shared
    // across every roster board (Mens/Boys/Girls/Womens, "make all pills
    // same since it's generic" per owner directive), see
  ...
  async onPositionSelectChange(select) {
    ...
    const cardEls = this.slotReorder(select, 'mr-card', 'mr-drop-zone');
    if (!cardEls) return;
